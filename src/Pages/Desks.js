...
import { Tabs, Typography, Popover, Form, Button, Input, Skeleton } from "antd";
import {
  LikeFilled,
  StarFilled,
  LockOutlined,
      ...
      <div>
        <Tabs
          ...
          onChange={(value) => {
            ...
            if (value == 2){
              ...
              dispatch(GetLikedDesksByUserId(userData.id));
            }
            if (value == 3){
              dispatch({type: "IS_LOADING"});
        ...
        >
          <TabPane tab="Your desks" key="1">
            { loadItem ?  <Skeleton/> : contentMyDesks(myDesks)}
            {myDesks.length === 0 ? <p>You don't have any desks!</p> : ""}
          </TabPane>
          <TabPane tab="Liked desks" key="2">
            {/* {contentMyDesks(likedDesks)} */}
            { loadItem ?  <Skeleton/> : contentMyDesks(likedDesks)}
            {likedDesks ? "":<p>You still haven't like any desk!</p>}
          </TabPane>
          <TabPane tab="History" key="3">
            {/* {contentMyDesks(historyDesks)} */}
            { loadItem ?  <Skeleton/> : contentMyDesks(historyDesks)}
            {historyDesks ? "":<p>You still haven't like any desk!</p>}
          </TabPane>
        </Tabs>
      </div>
    </div>
